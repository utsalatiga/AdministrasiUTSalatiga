-- ============================================================================
-- SCRIPT SQL BERSIH UNTUK MEMPERBAIKI RPC process_manual_payment & RELOAD CACHE
-- ============================================================================

-- 1. Hapus semua variasi fungsi lama (Overloaded Functions) agar tidak bentrok
DROP FUNCTION IF EXISTS public.process_manual_payment(UUID, BIGINT, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS public.process_manual_payment(UUID, BIGINT, TEXT, TEXT, TEXT, TEXT, TEXT, BIGINT);
...
CREATE OR REPLACE FUNCTION public.process_manual_payment(
  ...
  p_bukti_url TEXT,
  p_order_id TEXT,
  p_nominal_deposit BIGINT DEFAULT 0,
  p_no_kwitansi TEXT DEFAULT NULL
) RETURNS VOID AS $$
DECLARE
  v_mahasiswa_id UUID;
...
BEGIN
  ...
  -- 3. Catat transaksi pembayaran ke tabel pembayaran
...
-- 3. Paksa Supabase / PostgREST memuat ulang cache skema database
